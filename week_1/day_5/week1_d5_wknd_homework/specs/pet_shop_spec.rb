...
class TestPetShop < Minitest::Test
  ...
  def test_add_or_remove_cash__add
    ...
    # using earlier defined function total_cash
    # to return actual  cash value after we run add_or_remove_cash
    #
  ...
  def test_increase_pets_sold
    ...
    # using earlier defined function pets_sold
    # to return actual value after we run
    #increase_pets_sold, which does not return
    # anything
  ...
  def test_all_pets_by_breed__found
    ...
    # pets_by_breed returns an array assigned to pets var.
    # we the call .count to count how many breeds are in that
    # array, this is essentialy an actual result (pets.count)
  ...
  def test_find_pet_by_name__returns_nil
    ...
    # new assertion method, test passed if test returns nil
    assert_nil(pet)
  end

  def test_remove_pet_by_name
    remove_pet_by_name(@pet_shop, "Tristan")
    # use the earlier function to return pet hash or nil
    # assert_nil checks that ths function returns nil for
    # specific pet we removed with remove_pet_by_name
    pet = find_pet_by_name(@pet_shop,"Tristan")
    assert_nil(pet)
    # eq to assert_equal(nil, pet)
  end
  ...
  def test_remove_customer_cash
    # assign a chosen customer hash from customers array t0  customer variable for testing
  ...
  def test_customer_pet_count
    # this time customer variable not used but
    # expression @customers[0] is fed directly to
    # the count function - no diff..
  ...
  def test_add_pet_to_customer
    customer = @customers[0]
    # new pet var is pointed to hash as part of setup method
  ...
  def test_sell_pet_to_customer__pet_found
    # based on 0th customer(hsh) in customers array

    customer = @customers[0]

    pet = find_pet_by_name(@pet_shop,"Arthur")
    # based on wanted pet { name: "Arthur", pet_type: :dog, breed: "Husky", price: 900}
